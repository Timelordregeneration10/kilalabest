...
+ login形式的sticky实现
    + 竖型=>实现更复杂，需要改变scrollRef的height，不然根本sticky不起来，结束时会露底，改变麻烦，露底是因为content不能overflow-hidden, 否则stikcy失效
    + 横型=>实现简单，不需要改变scrollRef的height，但是结束时上面的仍然sticky影响观感 => 整个sticky包起来就行
...
比如kilalabest.cn的attempt的sticky中，由于滚动容器是w-screen h-screen的那个元素，而footer在滚动容器外，所以滚轮必须停下来且在滚动容器内才能滚动内部的东西=>现在没了，丝滑滚动
